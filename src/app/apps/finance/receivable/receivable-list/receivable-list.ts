...
export class ReceivableList {
  ...
  protected readonly invoices = computed<SalesInvoice[]>(() =>
    this.invoicingState.invoices().filter((invoice): invoice is SalesInvoice => invoice.documentType === 'sales'),
  );
  ...
  protected readonly groupedSections = computed<{ label: string; rows: SalesInvoice[] }[] | null>(() => {
    const field = this.groupBy();
    if (field === 'none') return null;
    const rows = this.filteredRows();
    const groups = new Map<string, SalesInvoice[]>();
    for (const row of rows) {
      const key = field === 'status' ? this.statusLabel(row.status) : row.customerName;
  ...
  protected isLate(invoice: SalesInvoice): boolean {
    return !SETTLED_STATUSES.includes(invoice.status) && new Date(invoice.dueDate) < new Date(TODAY);
  }

  protected isSettled(invoice: SalesInvoice): boolean {
