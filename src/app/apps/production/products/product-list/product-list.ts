...
const STATUS_OPTIONS: { value: ProductStatus; label: string }[] =
  (Object.keys(PRODUCT_STATUS_LABEL) as ProductStatus[]).map((value) => ({
    value,
    label: PRODUCT_STATUS_LABEL[value],
  }));
...
export class ProductList {
  ...
  protected readonly views: ListViewOption[] = [
    LIST_VIEW_OPTIONS.list,
    LIST_VIEW_OPTIONS.grid,
  ];
  ...
  protected readonly columns: DataTableColumn[] = [
    ...
    { key: 'actions', header: 'Acciones', width: '190px' },
  ];
  ...
  protected readonly selectedBom = computed(() => {
    ...
    return product ? this.productBom(product) : undefined;
  });
  ...
  protected productBom(product: Product) {
    const versions = this.productionState
      .billsOfMaterials()
      .filter((b) => b.productId === product.id);

    return (
      versions.find((b) => b.status === 'active') ??
      versions.sort((a, b) => (a.effectiveFrom < b.effectiveFrom ? 1 : -1))[0]
    );
  }

  protected prepareRecipeModal(event: Event, product: Product): void {
  ...
  protected workCenterLabel(workCenterId?: string): string {
    ...
    return (
      this.productionState
        .workCenters()
        .find((w) => w.id === workCenterId)?.name ?? workCenterId
    );
  }
